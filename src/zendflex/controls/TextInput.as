/*
 * Classe Original : MasterTextInput - FlexPernambuco
 */
package zendflex.controls
{
	...
	import zendflex.assets.AssetsClass;
	import zendflex.consts.TextCaps;
	import zendflex.consts.TextLang;
	import zendflex.consts.TextRestrict;
	import zendflex.consts.TextValidator;
	import zendflex.validators.ValidatorError;
	...
	public class TextInput extends mx.controls.TextInput
	{
		...
		// Clear Button Image
		private var clearButton:Image;
		
		// Contants - Clear Button Constant
		[Embed(source="../assets/img/icon/cross.png")]
		 ...
		 * @private
		 */
		private var _capsType:String = TextCaps.NORMAL;
		
		/**
		 * <p>Use the consts class <code>zendflex.consts.TextCaps</code>
		 * to set the lang settings.</p>
		 *
		 * @see zendflex.consts.TextCaps
		 *
		 * @default TextCaps.NORMAL
		 */
		...
		{
			switch (value) {
				case TextCaps.NORMAL:
				{
					break;
				}
				case TextCaps.LOWERCASE:
				{
					this.text = this.text.toLowerCase();
					break;
				}
				case TextCaps.UPPERCASE:
				{
				...
				default:
				{
					...
					var message:String = resourceManager.getString("rpc", "invalidResultFormat", [value,
						TextCaps.NORMAL, TextCaps.LOWERCASE, TextCaps.UPPERCASE, TextCaps.INIT]);
		 ...
		 * @private
		 */
		private var _onlyRestrict:String = TextRestrict.NONE;
		
		/**
		 * <p>Use the consts class <code>zendflex.consts.TextRestrict</code>
		 * or use the same logic of <code>TextInput.restrict</code> property.</p>
		 *
		 * @see zendflex.consts.TextRestrict
		 *
		 * @default TextRestrict.NONE
		 */
		 ...
		 * @private
		 */
		private var _validationLanguage:String = TextLang.PT_BR;
		
		/**
		 * <p>Use the consts class <code>zendflex.consts.TextLang</code>
		 * to set the lang settings.</p>
		 *
		 * @see zendflex.consts.TextLang
		 *
		 * @default TextLang.PT_BR
		 */
		...
		public function set validationLanguage(value:String):void
		{
			switch (value) {
				...
				{
					var message:String = resourceManager.getString("rpc", "invalidResultFormat", [value,
						TextLang.PT_BR, TextLang.ENG, TextLang.SPA]);
					throw new ArgumentError(message);
				}
			}
		 ...
		 * @private
		 */
		private var _validator:String = TextValidator.NONE;
		
		/**
		 * <p>Use the consts class <code>zendflex.consts.TextValidator</code>
		 * to set the lang settings.</p>
		 *
		 * @see zendflex.consts.TextValidator
		 *
		 * @default TextValidator.NONE
		 */
		...
		public function set validator(value:String):void
		{
			...
			switch (value) {
				...
				case TextValidator.PHONE:
				{
					setPhoneValidator();
					break;
				}
				case TextValidator.STRING:
				{
					setStringValidator();
					break;
				}
				case TextValidator.NUMBER:
				{
					setNumberValidator();
					break;
				}
				case TextValidator.DATE:
				{
					setDateValidator();
					break;
				}
				...
				{
					var message:String = resourceManager.getString("rpc", "invalidResultFormat", [value,
						TextValidator.DATE, TextValidator.EMAIL, TextValidator.NONE,
						TextValidator.NUMBER, TextValidator.PHONE, TextValidator.STRING]);
					throw new ArgumentError(message);
				}
			}
		...
		private function addClearButton(event:FlexEvent):void
		{
			if (clearButton == null) {
				...
				clearButton.x = this.width - 16 - (this.height - 16) / 2;
				clearButton.y = (this.height - 16) / 2;
				clearButton.source = AssetsClass.iconCross;
		...
		private var _blankMaskChar:String = " "; //_
